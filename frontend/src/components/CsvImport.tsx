  ...
  return (
    <Card className="p-5">
      ...
      <p className="mt-1 text-sm text-slate-500">
        ...
        <a href={CSV_TEMPLATE_URL} className="font-medium text-indigo-600 hover:underline">
          Download template
        </a>
      </p>

      <label className="mt-3 inline-flex cursor-pointer items-center rounded-lg border border-slate-300 px-3 py-1.5 text-sm font-medium text-slate-700 hover:bg-slate-50">
        ...
        <div className="mt-3 text-sm">
          <p className="text-slate-700">
            ...
            {result.failed > 0 && (
              <>
                {' '}
                · <span className="font-semibold text-red-600">{result.failed} skipped</span>
              </>
            )}
          </p>
          {result.errors.length > 0 && (
            <ul className="mt-1 space-y-0.5 text-xs text-red-700">
              ...
              {result.errors.length > 8 && (
                <li className="text-slate-500">…and {result.errors.length - 8} more</li>
              )}
            </ul>
          )}
        </div>
